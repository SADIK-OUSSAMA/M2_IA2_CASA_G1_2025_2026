class Snake extends Vehicle {
    ...
    move(target, obstacles) {
        ...
        let forceArrive = this.head.arrive(target);
        let forceAvoid = this.head.avoid(obstacles);

        // On multiplie les forces par leurs poids respectifs
        forceArrive.mult(this.seekWeight);
        forceAvoid.mult(this.avoidWeight);

        // On applique les forces
        this.head.applyForce(forceArrive);
